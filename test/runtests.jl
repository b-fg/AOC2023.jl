using AdventOfCode2023, Test

@testset "Day 01" begin
    @test AdventOfCode2023.Day01.day01() == (54304, 54418)
end

@testset "Day 02" begin
    @test AdventOfCode2023.Day02.day02() == (2563, 70768)
end

@testset "Day 03" begin
    @test AdventOfCode2023.Day03.day03() == (507214, 72553319)
end

@testset "Day 04" begin
    @test AdventOfCode2023.Day04.day04() == (15205, 6189740)
end

@testset "Day 05" begin
    @test AdventOfCode2023.Day05.day05() == (157211394, 50855035)
end

@testset "Day 06" begin
    @test AdventOfCode2023.Day06.day06() == (633080, 20048741)
end

@testset "Day 07" begin
    @test AdventOfCode2023.Day07.day07() == (253954294, 254837398)
end

@testset "Day 08" begin
    @test AdventOfCode2023.Day08.day08() == (16531, 24035773251517)
end

@testset "Day 09" begin
    @test AdventOfCode2023.Day09.day09() == (1647269739, 864)
end

@testset "Day 10" begin
    @test AdventOfCode2023.Day10.day10() == (6886, 371)
end

@testset "Day 11" begin
    @test AdventOfCode2023.Day11.day11() == (9418609, 593821230983)
end

@testset "Day 13" begin
    @test AdventOfCode2023.Day13.day13() == (32371, 37416)
end